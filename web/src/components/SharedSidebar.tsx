  ...
  return (
    <>
      ...
      <aside className={`sidenav ${open ? "open" : ""}`}>
        ...
        <button
          className={`dropdown-btn ${boardsOpen ? "active" : ""}`}
          onClick={() => setBoardsOpen(!boardsOpen)}
        >
          📁 게시판 모음 <span>▼</span>
        </button>
        {boardsOpen && (
          <div className="dropdown-container visible">
            <Link href="/notice">📢 공지 게시판</Link>
            {role !== "teacher" && (
              <>
                <Link href="/free">💬 자유 게시판</Link>
                <Link href="/anonymous">👻 익명 게시판</Link>
              </>
            )}
          </div>
        )}
        <button
          className={`dropdown-btn ${lostOpen ? "active" : ""}`}
        ...
        {role === "admin" && (
          <Link href="/admin_dashboard">👑 회원가입 신청</Link>
        )}
        {role !== "teacher" && (
          <a href="https://diep.io/" target="_blank" rel="noreferrer">
            🔍 Diep.io
          </a>
        )}
        <a href="https://sshs.app/school/food" target="_blank" rel="noreferrer">
          🍔 급식 및 간식
        </a>
        <Link href="/english">📖 영어 단어 시험</Link>
        <button
          className={`dropdown-btn ${quizzesOpen ? "active" : ""}`}
        ...
        <hr />
        <Link className="outing-link" href="/outing">
          📍 이석 현황/신청
        </Link>
        <Link href="/continuous_outing">🔄 연속 이석 신청</Link>
        <Link className="google-link" href="/google_login">
          🔑 학교 Google 로그인
        </Link>
      </aside>
    </>
