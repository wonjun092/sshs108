...
export function MyPageClient() {
  ...
  return <div className="card"><h2>비밀번호 변경</h2><form className="board-form" onSubmit={submit}><input name="currentPassword" type="password" placeholder="현재 비밀번호" required /><input name="newPassword" type="password" minLength={5} placeholder="새 비밀번호" required /><input name="confirmPassword" type="password" minLength={5} placeholder="새 비밀번호 확인" required />{error && <p className="error-message">{error}</p>}{message && <p className="success-message">{message}</p>}<button>비밀번호 변경</button></form></div>;
}
