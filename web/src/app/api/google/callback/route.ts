...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || state !== request.cookies.get("sshs108_google_state")?.value) return outingError(request, "invalid_state");
  ...
  const response = NextResponse.redirect(new URL("/outing", request.url)); response.cookies.set(googleCookieName(), signed, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", maxAge: 60 * 60 * 24 * 30, path: "/" }); response.cookies.delete("sshs108_google_state"); return response;
}
