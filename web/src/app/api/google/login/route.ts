...
export async function GET(request: NextRequest) {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json({ error: "Vercel Preview 환경에 Google OAuth 환경변수가 설정되지 않았습니다." }, { status: 503 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: "GOOGLE_REDIRECT_URI 또는 APP_URL이 올바른 URL이 아닙니다." }, { status: 503 });
  }
  ...
  const response = NextResponse.redirect(url); response.cookies.set("sshs108_google_state", state, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", maxAge: 600, path: "/" }); return response;
}
