...
export async function PATCH(request: Request) {
  try {
    ...
    if (!user || !(await compare(currentPassword, user.passwordHash))) return NextResponse.json({ error: "현재 비밀번호가 틀렸습니다." }, { status: 400 });
    user.passwordHash = await hash(newPassword, 12); await user.save();
